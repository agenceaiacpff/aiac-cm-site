...
import OperationsPanel, { BeneficiaryRow, DocumentRow, ProjectMemberRow, ProjectRow, TaskRow } from "@/components/OperationsPanel";
...
type RequestRow = { id:string; subject:string; request_type:string; status:string; priority:string; created_at:string; created_by?:string; assigned_to?:string|null; project_id?:string|null };
...
export default function PortalClient({ profile, initialRequests, initialConversations, initialNotifications, staffProfiles, initialAuditLogs, initialProjects, initialProjectMembers, initialTasks, initialDocuments, initialBeneficiaries }:{ profile:Profile; initialRequests:RequestRow[]; initialConversations:Conversation[]; initialNotifications:Notification[]; staffProfiles:Profile[]; initialAuditLogs:AuditLog[]; initialProjects:ProjectRow[]; initialProjectMembers:ProjectMemberRow[]; initialTasks:TaskRow[]; initialDocuments:DocumentRow[]; initialBeneficiaries:BeneficiaryRow[] }) {
  ...
  return <div className="portalShell">
    <aside className="portalSidebar"><a href="/" className="portalBrand"><img src="/aiac-logo.bmp" alt="AIAC"/><span><b>AIAC</b><small>Portail communautaire</small></span></a><nav>
      {[["accueil","Tableau de bord"],["demandes","Mes demandes"],["messages","Messagerie"],["notifications","Notifications"],["profil","Mon profil"],...(isStaff?[["operations","Gestion opérationnelle"]]:[]),...(isAdmin?[["administration","Administration"]]:[]),...(isSuperAdmin?[["audit","Journal d’audit"]]:[])].map(([id,label])=><button key={id} className={tab===id?"active":""} onClick={()=>setTab(id)}>{label}</button>)}
    ...
    <main className="portalMain"><header><div><p className="eyebrow">{labels[profile.role] || profile.role}</p><h1>Bonjour, {profile.full_name || "membre AIAC"}</h1></div><span className={`status ${profile.status}`}>{profile.status === "active" ? "Compte actif" : "Validation en attente"}</span></header>
      ...
      {tab==="operations"&&isStaff&&<OperationsPanel profile={profile} initialProjects={initialProjects} initialMembers={initialProjectMembers} initialTasks={initialTasks} initialDocuments={initialDocuments} initialBeneficiaries={initialBeneficiaries} initialRequests={requests} staffProfiles={staffProfiles} />}
