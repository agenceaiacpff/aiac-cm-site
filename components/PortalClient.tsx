...
import FieldReportingPanel, { ActivityTaskCountRow, ActivityTaskRow, TaskReportApprovalRow, TaskReportAttendanceRow, TaskReportEventRow, TaskReportEvidenceRow, TaskReportIndicatorRow, TaskReportRow } from "@/components/FieldReportingPanel";
...
export default function PortalClient({
  ...
  initialActivityTasks,initialActivityTaskCounts,initialTaskReports,initialTaskReportEvidence,initialTaskReportAttendance,initialTaskReportIndicators,initialTaskReportApprovals,initialTaskReportEvents,
}:{
  ...
  async function uploadAvatar(event:FormEvent<HTMLFormElement>){
    event.preventDefault();setAvatarBusy(true);const form=event.currentTarget;const file=new FormData(form).get("avatar");
    if(!(file instanceof File)||file.size===0){setNotice("Sélectionnez une photo.");setAvatarBusy(false);return;}
    if(file.size>5*1024*1024){setNotice("La photo dépasse la limite de 5 Mo.");setAvatarBusy(false);return;}
    const extensions:Record<string,string>={"image/jpeg":"jpg","image/png":"png","image/webp":"webp"};const extension=extensions[file.type];
    if(!extension){setNotice("Formats autorisés : JPG, PNG ou WebP.");setAvatarBusy(false);return;}
    const objectPath=`${profile.id}/profile.${extension}`;const uploaded=await supabase.storage.from("aiac-avatars").upload(objectPath,file,{contentType:file.type,upsert:true,cacheControl:"3600"});
    if(uploaded.error){setNotice(uploaded.error.message);setAvatarBusy(false);return;}
    const publicUrl=supabase.storage.from("aiac-avatars").getPublicUrl(objectPath).data.publicUrl;const versionedUrl=`${publicUrl}?v=${Date.now()}`;const {error}=await supabase.from("profiles").update({avatar_url:versionedUrl}).eq("id",profile.id);
    if(error){setNotice(error.message);setAvatarBusy(false);return;}setAvatarUrl(versionedUrl);form.reset();setNotice("Photo de profil mise à jour.");setAvatarBusy(false);
  }
  async function removeAvatar(){
    setAvatarBusy(true);const {data}=await supabase.storage.from("aiac-avatars").list(profile.id);const paths=(data||[]).map((item:{name:string})=>`${profile.id}/${item.name}`);if(paths.length)await supabase.storage.from("aiac-avatars").remove(paths);
    const {error}=await supabase.from("profiles").update({avatar_url:null}).eq("id",profile.id);if(error)setNotice(error.message);else{setAvatarUrl("");setNotice("Photo de profil supprimée.");}setAvatarBusy(false);
  }
  async function logout(){await supabase.auth.signOut();router.push("/connexion");router.refresh();}

  const navItems=[["accueil","Tableau de bord"],["reunions","Réunions et agenda"],["demandes","Mes demandes"],["messages","Messagerie"],["notifications","Notifications"],["annonces","Annonces"],["profil","Mon profil"],...(canFieldReport?[["terrain","Programmes et rapports"]]:[]),...(isStaff?[["documents","Documents sécurisés"],["operations","Gestion opérationnelle"],["institution","Gestion institutionnelle"],["contenus","Publications du site"]]:[]),...(isAdmin?[["administration","Administration"]]:[]),...(isSuperAdmin?[["audit","Journal d’audit"]]:[])];
  ...
  return <div className="portalShell">
    <aside className="portalSidebar"><a href="/nouveau-site/index.html" className="portalBrand"><img src="/aiac-logo.bmp" alt="AIAC"/><span><b>AIAC</b><small>Site public</small></span></a><div className="portalIdentity">{avatarUrl?<img className={profileStyles.sidebarAvatar} src={avatarUrl} alt={`Photo de ${profile.full_name||"profil"}`}/>:<span aria-hidden="true">{(profile.full_name||profile.email||"A").charAt(0).toUpperCase()}</span>}<div><b>{profile.full_name||"Membre AIAC"}</b><small>{roleLabels[profile.role]||profile.role}</small></div></div><nav>{navItems.map(([id,label])=><button key={id} className={tab===id?"active":""} onClick={()=>setTab(id)}><span>{label}</span>{navCounts[id]>0&&<i className="navBadge" aria-label={`${navCounts[id]} éléments non lus`}>{countLabel(navCounts[id])}</i>}</button>)}</nav><a className="publicSiteLink" href="/nouveau-site/index.html">Voir le site public</a><button className="logout" onClick={logout}>Se déconnecter</button></aside>
    <main className="portalMain"><header><div><p className="eyebrow">{roleLabels[profile.role]||profile.role}</p><h1>Bonjour, {profile.full_name||"membre AIAC"}</h1></div><div className="portalHeaderStatus"><span className={`realtimeStatus ${realtimeConnected?"connected":"connecting"}`}>{realtimeConnected?"● Synchronisation en direct":"● Reconnexion…"}</span><span className={`status ${profile.status}`}>{profile.status==="active"?"Compte actif":profile.status==="suspended"?"Compte suspendu":"Validation en attente"}</span></div></header>
      ...
      {tab==="terrain"&&canFieldReport&&<FieldReportingPanel profile={profile} programs={initialPrograms} projects={initialProjects} activities={initialActivities} projectMembers={initialProjectMembers} staffProfiles={staffProfiles} bodies={initialBodies} workforceAssignments={initialWorkforceAssignments} positionAssignments={initialPositionAssignments} institutionalMembers={initialInstitutionalMembers} bodyMemberships={initialBodyMemberships} initialActivityTasks={initialActivityTasks} initialActivityTaskCounts={initialActivityTaskCounts} initialTaskReports={initialTaskReports} initialEvidence={initialTaskReportEvidence} initialAttendance={initialTaskReportAttendance} initialIndicators={initialTaskReportIndicators} initialApprovals={initialTaskReportApprovals} initialEvents={initialTaskReportEvents}/>}
