...
export default function PortalClient({
  ...
  initialTaskReports,
  initialTaskReportEvidence,
  initialTaskReportAttendance,
  ...
  >(() =>
    Object.fromEntries(
      initialUnreadMessageCounts.map((item) => [
  ...
  const [requestedConversationId, setRequestedConversationId] = useState<
    string | null
  >(null);
  const [requestedMeetingId, setRequestedMeetingId] = useState<string | null>(
    null,
  );
  const [meetingOverview, setMeetingOverview] = useState(initialMeetings);
  const [meetingInvitationOverview, setMeetingInvitationOverview] = useState(
    initialMeetingParticipants,
  );
  ...
  const refreshUnreadMessageCounts = useCallback(async () => {
    ...
    if (!error && data)
      setUnreadMessageCounts(
        Object.fromEntries(
          (data as UnreadMessageCountRow[]).map((item) => [
            item.conversation_id,
            Number(item.unread_count),
          ]),
        ),
  ...
  const refreshMeetingOverview = useCallback(async () => {
    const [{ data: meetingRows }, { data: participantRows }] =
      await Promise.all([
    ...
    if (participantRows)
      setMeetingInvitationOverview(participantRows as MeetingParticipantRow[]);
  }, [supabase]);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const requested = params.get("tab");
    if (requested && portalTabs.includes(requested)) setTab(requested);
    setRequestedConversationId(params.get("conversation"));
    setRequestedMeetingId(params.get("meeting"));
  }, []);
  useEffect(() => {
    const channel = supabase
      .channel(`portal-live:${profile.id}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const unreadMessages = Object.values(unreadMessageCounts).reduce(
  ...
  const unreadAnnouncements = initialAnnouncements.filter(
    (item) =>
      ...
      !initialAnnouncementReadIds.includes(item.id),
  ).length;
  ...
  async function removeAvatar() {
    ...
    const paths = (data || []).map(
      (item: { name: string }) => `${profile.id}/${item.name}`,
    );
  ...
  const navItems = [
    ...
    ["annonces", "Annonces"],
    ["profil", "Mon profil"],
    ...(canFieldReport ? [["terrain", "Cycle des programmes"]] : []),
    ...(isStaff
      ? [
          ["operations", "Gestion opérationnelle"],
          ["institution", "Gestion institutionnelle"],
          ["documents", "Documents sécurisés"],
          ["contenus", "Publications du site"],
        ]
      : []),
    ...(isAdmin ? [["administration", "Comptes et accès"]] : []),
    ...(isSuperAdmin
      ? [
    ...
    <div className="portalShell">
      <aside className="portalSidebar">
        <a href="/nouveau-site/index.html" className="portalBrand">
          ...
          <span>
            <b>AIAC</b>
            <small>Site public</small>
          </span>
        </a>
        <div className="portalIdentity">
          {avatarUrl ? (
            <img
              className={profileStyles.sidebarAvatar}
              src={avatarUrl}
          ...
          {navItems.map(([id, label]) => (
            ...
            >
              ...
              {navCounts[id] > 0 && (
                <i
                  className="navBadge"
                  aria-label={`${navCounts[id]} éléments non lus`}
                >
                  {countLabel(navCounts[id])}
                </i>
              )}
            </button>
          ))}
        </nav>
        <a className="publicSiteLink" href="/nouveau-site/index.html">
          Voir le site public
        </a>
      ...
      <main className="portalMain">
        <header>
          ...
          <div className="portalHeaderStatus">
            <span
              className={`realtimeStatus ${realtimeConnected ? "connected" : "connecting"}`}
            >
              {realtimeConnected
          ...
          <section>
            <div className="statGrid">
              ...
              <article>
                ...
                <span>Annonces à lire</span>
              </article>
            </div>
            <div className="portalPanel">
              <h2>Bienvenue dans votre espace AIAC</h2>
              <p>
                Soumettez et suivez vos demandes, échangez avec l’équipe,
                organisez vos réunions, consultez les annonces et gérez votre
                profil depuis cet espace sécurisé.
              </p>
            </div>
          </section>
        )}
        {tab === "reunions" && (
          <MeetingsPanel
            profile={profile}
            initialMeetings={meetingOverview}
            initialParticipants={meetingInvitationOverview}
            initialGuests={initialMeetingGuests}
            recipients={meetingRecipients}
            bodies={meetingBodies}
            projects={meetingProjects}
            initialSelectedId={requestedMeetingId}
          />
        )}
        {tab === "demandes" && (
          <RequestsPanel
            ...
            initialEvents={initialRequestEvents}
          />
        )}
        {tab === "messages" && (
          <MessageCenter
            ...
            initialActiveId={requestedConversationId}
            unreadCounts={unreadMessageCounts}
            onConversationRead={refreshUnreadMessageCounts}
            onConversationChange={mergeConversation}
            recipients={messageRecipients}
            bodies={initialBodies}
          />
        )}
        {tab === "documents" && isStaff && (
          <DocumentVault
            profile={profile}
            staffProfiles={staffProfiles}
            initialDocuments={initialDocuments}
            initialFolders={initialDocumentFolders}
            initialVersions={initialDocumentVersions}
            initialApprovals={initialDocumentApprovals}
            initialGrants={initialDocumentGrants}
            bodies={initialBodies}
            projects={initialProjects}
            beneficiaries={initialBeneficiaries}
            cases={initialCaseFiles}
            partners={initialPartners}
            activities={initialActivities}
            members={initialInstitutionalMembers}
          />
        )}
        {tab === "notifications" && (
          <NotificationsPanel
        ...
        {tab === "annonces" && (
          <AnnouncementsPanel
            ...
            initialReadIds={initialAnnouncementReadIds}
          />
        )}
        {tab === "profil" && (
          <section className="portalPanel">
            ...
            <div className={profileStyles.profileGrid}>
              <div className={profileStyles.avatarCard}>
                {avatarUrl ? (
                  ...
                  <div
                    className={profileStyles.avatarFallback}
                    aria-hidden="true"
                  >
                    {(profile.full_name || profile.email || "A")
                      .charAt(0)
                ...
                <div>
                  ...
                  <form
                    className={profileStyles.avatarForm}
                  ...
                  >
                    <input
                      ...
                      required
                    />
                    <button disabled={avatarBusy}>
                      {avatarBusy ? "Envoi…" : "Choisir et enregistrer"}
                    </button>
                  </form>
                  {avatarUrl && (
                    <button
            ...
            {isAdmin && (
              <div className="securityBox">
                <h3>Sécurité renforcée active</h3>
                <p>
                  Les actions administratives sensibles exigent une session MFA
                  de niveau AAL2.
                </p>
                <a href="/mfa">Vérifier mon authentification</a>
              </div>
            )}
          </section>
        )}
        {tab === "operations" && isStaff && (
          <OperationsPanel
            profile={profile}
            initialProjects={initialProjects}
            initialPrograms={initialPrograms}
            initialActivities={initialActivities}
            initialMembers={initialProjectMembers}
            initialTasks={initialTasks}
            initialDocuments={initialDocuments}
            initialBeneficiaries={initialBeneficiaries}
            initialRequests={requests}
            initialRequestEvents={initialRequestEvents}
            initialTaskEvents={initialTaskEvents}
            initialInterventions={initialInterventions}
            staffProfiles={staffProfiles}
            bodies={initialBodies}
          />
        )}
        {tab === "terrain" && canFieldReport && (
          <FieldReportingPanel
            profile={profile}
            ...
            staffProfiles={staffProfiles}
            bodies={initialBodies}
            workforceAssignments={initialWorkforceAssignments}
            positionAssignments={initialPositionAssignments}
            institutionalMembers={initialInstitutionalMembers}
            bodyMemberships={initialBodyMemberships}
            initialActivityTasks={initialActivityTasks}
            initialActivityTaskCounts={initialActivityTaskCounts}
            initialTaskReports={initialTaskReports}
            initialEvidence={initialTaskReportEvidence}
            initialAttendance={initialTaskReportAttendance}
        ...
        {tab === "institution" && isStaff && (
          <InstitutionalPanel
            ...
            initialInstitutionalMembers={initialInstitutionalMembers}
            initialBodyMemberships={initialBodyMemberships}
            initialWorkforceAssignments={initialWorkforceAssignments}
            initialPrograms={initialPrograms}
            initialPartners={initialPartners}
            initialPartnerships={initialPartnerships}
            initialCaseFiles={initialCaseFiles}
            initialCaseNotes={initialCaseNotes}
            initialCaseActions={initialCaseActions}
            initialActivities={initialActivities}
            initialActivityReports={initialActivityReports}
          />
        )}
        {tab === "contenus" && isStaff && (
          <PublicContentPanel
            profileId={profile.id}
            bodies={initialBodies.filter((body) =>
              manageablePublicBodyIds.includes(body.id),
            )}
            initialItems={initialPublicContent}
            initialMedia={initialPublicMedia}
            initialGuestbook={initialGuestbookEntries}
            projects={initialProjects}
            programs={initialPrograms}
        ...
        {tab === "administration" && isAdmin && (
          <AccountsPanel
            ...
            initialHistory={initialAccountHistory}
            initialBodies={initialBodies}
            initialPositions={initialPositionDefinitions}
            initialPositionAssignments={initialPositionAssignments}
            initialReviews={initialAccountReviews}
        ...
        {tab === "data-control" && isSuperAdmin && (
          <SuperAdminDataCenter
            programs={initialPrograms}
            projects={initialProjects}
            activities={initialActivities}
            tasks={initialActivityTasks}
            reports={initialTaskReports}
            publications={initialPublicContent}
            documents={initialDocuments}
            bodies={initialBodies}
            members={initialInstitutionalMembers}
            workforce={initialWorkforceAssignments}
            partners={initialPartners}
          />
        )}
        {tab === "audit" && isSuperAdmin && (
          <AuditCenter
            ...
            sessions={initialSessionActivity}
            documentAccess={initialDocumentAccessLogs}
