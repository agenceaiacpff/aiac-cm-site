...
const eventLabels:Record<string,string>={comment:"Réponse",status_change:"Statut modifié",information_request:"Informations complémentaires demandées",intervention_created:"Dossier d’intervention créé",reopened:"Demande rouverte"};
...
export default function RequestsPanel({profileId,requests,setRequests,initialEvents}:{profileId:string;requests:RequestRow[];setRequests:Dispatch<SetStateAction<RequestRow[]>>;initialEvents:WorkflowEvent[]}){
  ...
  async function addComment(event:FormEvent<HTMLFormElement>,requestId:string){
    ...
    const {data:created,error}=await supabase.from("request_events").insert({request_id:requestId,actor_id:profileId,event_type:"comment",body,visibility:"requester"}).select().single();
