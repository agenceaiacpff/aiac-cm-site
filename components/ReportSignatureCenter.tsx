...
export default function ReportSignatureCenter({profile}:{profile:Profile}){
 ...
 function choices(data:FormData,side:"left"|"right"){return{
  include_nominal_seal:data.get("nominal_seal")==="on",
  include_round_seal:data.get("round_seal")==="on",
  signature_block_side:side
 };}
 async function submit(event:FormEvent<HTMLFormElement>,report:Report){
  event.preventDefault();setBusy(report.id);setNotice("");const data=new FormData(event.currentTarget);const c=choices(data,"left");
  ...
  if(error)setNotice(error.message);else{setNotice(`${report.report_number} a été signé et soumis avec le bloc du rapporteur fixé à gauche.`);await reload();}
  setBusy("");
 }
 async function review(event:FormEvent<HTMLFormElement>,report:Report,decision:"approved"|"returned"){
  event.preventDefault();setBusy(report.id);setNotice("");const data=new FormData(event.currentTarget);const c=choices(data,"right");const comment=String(data.get("comment")||"").trim();
  ...
  if(error)setNotice(error.message);else{setNotice(decision==="approved"?`${report.report_number} a été validé avec le bloc d’approbation fixé à droite.`:`${report.report_number} a été retourné pour correction.`);await reload();}
  setBusy("");
 }
 ...
 const Picker=({side}:{side:"left"|"right"})=><fieldset className="securityBox"><legend><b>Éléments à apposer avec cette signature</b></legend><p>La signature officielle est toujours apposée. Les cachets ne sont ajoutés que si vous les laissez cochés pour cette signature.</p><label><input type="checkbox" checked readOnly/> Signature officielle</label>{hasNominal?<label><input name="nominal_seal" type="checkbox" defaultChecked/> Cachet nominatif</label>:<small>Cachet nominatif : aucun actif enregistré.</small>}{hasRound?<label><input name="round_seal" type="checkbox" defaultChecked/> Cachet rond</label>:<small>Cachet rond : aucun actif enregistré.</small>}<p><b>Position institutionnelle fixe :</b> {side==="left"?"auteur / rapporteur à gauche":"approbateur / valideur à droite"}.</p><small>Ordre automatique : identité et qualité, signature, cachet nominatif, cachet rond, puis date. La position n’est plus modifiable afin de conserver la même présentation dans tous les rapports.</small></fieldset>;
 return <section className="portalPanel"><div className="panelTitle"><div><p className="eyebrow">Signature institutionnelle</p><h2>Signer avec les éléments officiels</h2><p>Règle commune AIAC : <b>le rapporteur signe à gauche</b> et <b>l’approbateur signe à droite</b>. Chaque décision conserve les éléments cochés au moment de la signature.</p></div></div>{notice&&<div className="notice" role="status">{notice}</div>}{!hasSignature&&<div className="notice">Aucune signature officielle active n’est enregistrée pour votre compte.</div>}
 {own.map(r=><article className="reviewCard" key={r.id}><h3>{r.report_number} · {r.title||"Rapport"}</h3><p>{r.summary}</p><form className="reviewForm" onSubmit={e=>submit(e,r)}><Picker side="left"/><button className="approveButton" disabled={!hasSignature||busy===r.id}>{busy===r.id?"Signature…":"Signer et soumettre"}</button></form></article>)}
 {reviewable.map(r=><article className="reviewCard" key={r.id}><h3>{r.report_number} · {r.title||"Rapport"}</h3><p>{r.summary}</p><form className="reviewForm" onSubmit={e=>review(e,r,"approved")}><Picker side="right"/><textarea name="comment" placeholder="Observation — facultative en cas d’approbation"/><button className="approveButton" disabled={!hasSignature||busy===r.id}>{busy===r.id?"Validation…":"Valider et signer"}</button></form><details><summary>Retourner pour correction</summary><form className="reviewForm returnForm" onSubmit={e=>review(e,r,"returned")}><Picker side="right"/><textarea name="comment" minLength={5} required placeholder="Corrections demandées"/><label><input name="require_evidence" type="checkbox"/> Exiger une preuve à la prochaine soumission</label><button disabled={!hasSignature||busy===r.id}>Retourner et signer la décision</button></form></details></article>)}
 </section>;
}
