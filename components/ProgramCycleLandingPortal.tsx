  ...
  return (
    <div className="portalShell">
      <aside className="portalSidebar">
        ...
        <div className="portalIdentity"><span aria-hidden="true">{(profile.full_name || profile.email || "A").charAt(0).toUpperCase()}</span><div><b>{profile.full_name || "Membre AIAC"}</b><small>{roleLabels[profile.role] || profile.role}</small></div></div>
        <nav>
          {links.filter(([id]) => {
            ...
            return true;
          }).map(([id, label]) => <button type="button" key={id} className={id === "terrain" ? "active" : ""} onClick={() => openTab(id)}><span>{label}</span></button>)}
        </nav>
      ...
      <main className="portalMain">
        <header><div><p className="eyebrow">{roleLabels[profile.role] || profile.role}</p><h1>Cycle des programmes</h1></div><span className={`status ${profile.status}`}>{profile.status === "active" ? "Compte actif" : profile.status}</span></header>

        <ProgramCycleNotificationsSummary profileId={profile.id} />
