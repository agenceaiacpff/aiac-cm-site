...
export default async function GuestMeetingInvitation({params}:{params:Promise<{token:string}>}){
  ...
  const {data,error}=await supabase.rpc("get_guest_meeting_invitation",{p_token:token});
  ...
  return <main className={styles.page}><article className={styles.card}>
    ...
    {!cancelled&&<GuestMeetingResponse token={token} initialResponse={invitation.response_status}/>}<footer>Ce lien est personnel. Ne le transmettez pas à une autre personne.</footer>
  </article></main>;
}
