...
export default async function FullProgramCyclePage() {
  ...
  const reportIds = Array.from(new Set((taskReports || []).map((item) => item.id)));
  ...
  const [
    ...
    reportIds.length
      ? supabase.from("task_report_evidence").select("*").in("report_id", reportIds).order("created_at")
      : Promise.resolve({ data: [] }),
    reportIds.length
      ? supabase.from("task_report_attendance").select("*").in("report_id", reportIds).order("created_at")
      : Promise.resolve({ data: [] }),
    reportIds.length
      ? supabase.from("task_report_indicator_values").select("*").in("report_id", reportIds).order("created_at")
      : Promise.resolve({ data: [] }),
    reportIds.length
      ? supabase.from("task_report_approvals").select("*").in("report_id", reportIds).order("created_at")
      : Promise.resolve({ data: [] }),
    reportIds.length
      ? supabase.from("task_report_events").select("*").in("report_id", reportIds).order("created_at")
      : Promise.resolve({ data: [] }),
  ]);
