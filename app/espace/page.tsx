...
export default async function Espace() {
  ...
  const [{ data: profiles }, { data: auditLogs }, { data: projects }, { data: projectMembers }, { data: tasks }, { data: documents }, { data: beneficiaries }, { data: requestEvents }, { data: taskEvents }, { data: accountHistory }, { data: announcements }, { data: announcementReads }, { data: bodies }, { data: institutionalMembers }, { data: bodyMemberships }, { data: workforceAssignments }, { data: programs }, { data: partners }, { data: partnerships }, { data: caseFiles }, { data: caseNotes }, { data: caseActions }, { data: activities }, { data: activityReports }, { data: positionDefinitions }, { data: positionAssignments }, { data: accountReviews }, { data: messageRecipients }, { data: documentFolders }, { data: documentVersions }, { data: documentApprovals }, { data: documentGrants }, { data: sessionActivity }, { data: documentAccessLogs }, { data: permissions }, { data: permissionOverrides }, { data: accountScopes }, { data: interventions }, { data: publicContent }, { data: publicMedia }, { data: guestbookEntries }, { data: meetings }, { data: meetingParticipants }, { data: meetingGuests }, { data: meetingRecipients }, { data: meetingBodies }, { data: meetingProjects }] = await Promise.all([
    isStaff ? supabase.from("profiles").select("id,full_name,email,role,status,phone,organization,avatar_url,registration_state,validated_at,validated_by,rejection_reason,must_reset_password,email_verified_at").order("full_name") : Promise.resolve({ data: [] }),
    ...
    canFieldReport ? supabase.from("projects").select("id,code,name,description,program_id,status,location,start_date,end_date,budget_amount,budget_currency,created_by,updated_at").order("updated_at",{ascending:false}) : Promise.resolve({ data: [] }),
    canFieldReport ? supabase.from("project_members").select("project_id,user_id,member_role,joined_at") : Promise.resolve({ data: [] }),
    ...
    canFieldReport ? supabase.from("governance_bodies").select("*").order("name") : Promise.resolve({ data: [] }),
    canFieldReport ? supabase.from("institutional_members").select("*").order("full_name") : Promise.resolve({ data: [] }),
    canFieldReport ? supabase.from("body_memberships").select("*").order("start_date",{ascending:false}) : Promise.resolve({ data: [] }),
    canFieldReport ? supabase.from("workforce_assignments").select("*").order("start_date",{ascending:false}) : Promise.resolve({ data: [] }),
    canFieldReport ? supabase.from("programs").select("*").order("updated_at",{ascending:false}) : Promise.resolve({ data: [] }),
    ...
    canFieldReport ? supabase.from("activities").select("*").order("starts_at",{ascending:false}) : Promise.resolve({ data: [] }),
    ...
    canFieldReport ? supabase.from("position_assignments").select("id,position_id,body_id,profile_id,member_id,territory,decision_reference,start_date,end_date,status,appointed_by").order("start_date",{ascending:false}) : Promise.resolve({data:[]}),
    ...
    isStaff ? supabase.from("public_content_items").select("*").order("updated_at",{ascending:false}).limit(500) : Promise.resolve({data:[]}),
    isStaff ? supabase.from("public_content_media").select("*").order("sort_order").limit(2000) : Promise.resolve({data:[]}),
    isStaff ? supabase.from("guestbook_entries").select("*").order("created_at",{ascending:false}).limit(500) : Promise.resolve({data:[]}),
    ...
    supabase.rpc("list_meeting_recipients"),
    supabase.rpc("list_meeting_bodies"),
    supabase.rpc("list_meeting_projects")
  ]);
  const {data:taskReports}=canFieldReport?await supabase.from("task_reports").select("*").order("updated_at",{ascending:false}):{data:[]};
  const {data:institutionalSignatureAssets}=canFieldReport
    ? await supabase.from("institutional_signature_assets").select("*").order("created_at",{ascending:false})
    : {data:[]};
  const reportedTaskIds=Array.from(new Set((taskReports||[]).map(item=>item.task_id)));
  const [{data:activityTasks},{data:activityTaskCounts},{data:taskReportEvidence},{data:taskReportAttendance},{data:taskReportIndicators},{data:taskReportApprovals},{data:taskReportEvents}]=await Promise.all([
    canFieldReport&&reportedTaskIds.length?supabase.from("activity_tasks").select("*").in("id",reportedTaskIds).order("sequence_no"):Promise.resolve({data:[]}),
    canFieldReport?supabase.rpc("list_activity_task_counts"):Promise.resolve({data:[]}),
    canFieldReport?supabase.from("task_report_evidence").select("*").order("created_at"):Promise.resolve({data:[]}),
    canFieldReport?supabase.from("task_report_attendance").select("*").order("created_at"):Promise.resolve({data:[]}),
    canFieldReport?supabase.from("task_report_indicator_values").select("*").order("created_at"):Promise.resolve({data:[]}),
    canFieldReport?supabase.from("task_report_approvals").select("*").order("created_at"):Promise.resolve({data:[]}),
