...
export default async function Espace() {
  ...
  const [{data:activityTasks},{data:taskReports},{data:taskReportEvidence},{data:taskReportAttendance},{data:taskReportIndicators},{data:taskReportApprovals},{data:taskReportEvents}]=await Promise.all([
    canFieldReport?supabase.from("activity_tasks").select("*").order("sequence_no"):Promise.resolve({data:[]}),
    canFieldReport?supabase.from("task_reports").select("*").order("updated_at",{ascending:false}):Promise.resolve({data:[]}),
