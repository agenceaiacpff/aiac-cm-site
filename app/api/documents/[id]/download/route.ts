...
export async function GET(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  if(version?.size_bytes&&bytes.length!==Number(version.size_bytes))return NextResponse.json({error:"Contrôle d’intégrité échoué : taille du fichier différente de la version enregistrée"},{status:502});
  ...
  await supabase.from("document_access_logs").insert({document_id:id,version_id:version?.id||null,user_id:userId,action:"download",source_ip:forwarded,user_agent:request.headers.get("user-agent"),details:{version_number:version?.version_number||document.current_version,source:"storage_proxy",integrity_checked:true}});
