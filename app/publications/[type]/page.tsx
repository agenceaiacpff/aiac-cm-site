...
export default async function PublicListing({params,searchParams}:{params:Promise<{type:string}>;searchParams:Promise<{page?:string;organe?:string}>}){
  ...
  const {data:bodyData}=await supabase.from("governance_bodies").select("id,code,name,description,subsidiary_code,region,locality").eq("body_type","subsidiary_body").eq("status","active").order("code");
...
function Empty(){return <div className="publicEmpty"><h2>Aucune publication pour le moment</h2><p>Les contenus validés par les organes subsidiaires apparaîtront ici automatiquement.</p></div>}
